...
const GeometryModel = ({ geometry }) => {
    return (
        <mesh geometry={geometry} castShadow receiveShadow>
            <meshStandardMaterial color="#FF6A00" roughness={0.3} metalness={0.8} />
        </mesh>
    );
};
...
const CadWindow = ({ data, thoughts, retryInfo = {}, onClose, socket }) => {
    ...
    return (
        <div className="w-full h-full relative group friday-panel overflow-hidden rounded-lg">
            {/* Close Button */}
            ...
            <div className="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <button
                    ...
                    className="bg-black/10 hover:bg-black/20 text-orange-300 text-xs px-2 py-1 rounded border border-orange-900/10 backdrop-blur-sm"
                >
                    ITERATE
                </button>
                <button
                    ...
                    className="bg-black/10 hover:bg-black/20 text-orange-300 text-xs px-2 py-1 rounded border border-orange-900/10 backdrop-blur-sm flex items-center gap-1"
                >
            ...
            {(isIterating || (!data && data?.format !== 'loading')) && (
                <div className={`absolute inset-0 z-20 ${!data ? 'bg-black/90' : 'bg-black/80'} flex items-center justify-center p-4`}>
                    <div className="bg-black/10 rounded p-4 w-full max-w-sm pointer-events-auto shadow-[0_0_20px_rgba(255,138,0,0.18)] border border-orange-900/8">
                        <h4 className="neon-accent text-sm mb-2 font-mono">
                        ...
                        <textarea
                            ...
                            className="w-full bg-black/10 border border-orange-900/8 rounded p-2 text-white text-sm mb-3 focus:outline-none focus:border-orange-400 h-24 resize-none"
                            autoFocus
                            onKeyDown={(e) => {
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            {data && (
                                <button onClick={() => setIsIterating(false)} className="text-gray-400 text-xs hover:text-white px-2 py-1">Cancel</button>
                            )}
                            <button onClick={!data ? handleGenerate : handleIterate} disabled={isSending} className="px-3 py-1 rounded bg-orange-500 text-white text-xs">{isSending ? "Generating..." : (!data ? "Generate" : "Update")}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {data?.format === 'loading' && (
                <div className="absolute inset-y-0 right-0 w-2/5 p-4 bg-black/70 backdrop-blur-sm border-l border-orange-900/8 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                        <h4 className="neon-accent text-xs font-mono tracking-widest uppercase flex items-center gap-2">
            ...
            <div className="absolute bottom-2 left-2 text-[10px] text-orange-300/70 font-mono tracking-widest pointer-events-none">CAD_ENGINE_V2: {data?.format?.toUpperCase() || "READY"}</div>
        </div>
    );
};
