...
const SettingsWindow = ({
    ...
    return (
        <div className="absolute top-20 right-10 p-4 rounded-lg z-50 w-80 friday-panel">
            <div className="flex justify-between items-center mb-4 border-b border-cyan-900/50 pb-2">
                <h2 className="neon-accent font-bold text-sm uppercase tracking-wider">Settings</h2>
                <button onClick={onClose} className="text-orange-300 hover:text-orange-200">
                    <X size={16} />
                </button>
            </div>
            ...
            <div className="mb-6">
                <h3 className="neon-accent font-bold mb-3 text-xs uppercase tracking-wider opacity-90">Security</h3>
                <div className="flex items-center justify-between text-xs bg-black/10 p-2 rounded border border-orange-900/8">
                    <span className="text-gray-200">Face Authentication</span>
                    <button onClick={toggleFaceAuth} className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${faceAuthEnabled ? 'bg-orange-400/80' : 'bg-gray-700'}`}>
            ...
            {/* Microphone Section */}
            <div className="mb-4">
                <h3 className="neon-accent font-bold mb-2 text-xs uppercase tracking-wider opacity-90">Microphone</h3>
                <select
                    value={selectedMicId}
                    onChange={(e) => setSelectedMicId(e.target.value)}
                    className="w-full bg-black/10 border border-orange-900/8 rounded p-2 text-xs text-gray-100 focus:border-orange-400 outline-none"
                >
            ...
            {/* Speaker Section */}
            <div className="mb-4">
                <h3 className="neon-accent font-bold mb-2 text-xs uppercase tracking-wider opacity-90">Speaker</h3>
                <select
            ...
            {/* Webcam Section */}
            <div className="mb-6">
                <h3 className="neon-accent font-bold mb-2 text-xs uppercase tracking-wider opacity-90">Webcam</h3>
                <select
            ...
            <div className="mb-6">
                <div className="flex justify-between mb-2">
                    <h3 className="neon-accent font-bold text-xs uppercase tracking-wider opacity-90">Cursor Sensitivity</h3>
                    <span className="text-xs text-orange-300">{cursorSensitivity}x</span>
                </div>
                <input
                    type="range"
                    min="1.0"
                    ...
                    className="w-full accent-orange-400 cursor-pointer h-1 bg-gray-800 rounded-lg appearance-none"
                />
            </div>

            {/* Gesture Control Section */}
            <div className="mb-6">
                <h3 className="neon-accent font-bold mb-3 text-xs uppercase tracking-wider opacity-90">Gesture Control</h3>
                <div className="flex items-center justify-between text-xs bg-black/10 p-2 rounded border border-orange-900/8">
                    <span className="text-gray-200">Flip Camera Horizontal</span>
                    <button onClick={toggleCameraFlip} className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${isCameraFlipped ? 'bg-orange-400/80' : 'bg-gray-700'}`}>
            ...
            <div className="mb-6">
                <h3 className="neon-accent font-bold mb-3 text-xs uppercase tracking-wider opacity-90">Tool Confirmations</h3>
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                    {TOOLS.map(tool => {
                        const isRequired = permissions[tool.id] !== false; // Default True
                        return (
                            <div key={tool.id} className="flex items-center justify-between text-xs bg-black/10 p-2 rounded border border-orange-900/8">
                                <span className="text-gray-200">{tool.label}</span>
                                <button onClick={() => togglePermission(tool.id)} className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${isRequired ? 'bg-orange-400/80' : 'bg-gray-700'}`}>
