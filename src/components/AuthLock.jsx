...
const AuthLock = ({ socket, onAuthenticated, onAnimationComplete }) => {
    ...
    const themeColor = isUnlocking ? 'text-green-500' : 'neon-accent';
    const borderColor = isUnlocking ? 'border-green-500' : 'border-orange-900';
    const shadowColor = isUnlocking ? 'shadow-[0_0_50px_rgba(34,197,94,0.4)]' : 'shadow-[0_0_50px_rgba(255,138,0,0.28)]';
    ...
    return (
        ...
        <div className={`fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center font-mono select-none transition-all duration-[2000ms] ${isUnlocking ? 'opacity-0 scale-110 pointer-events-none' : 'opacity-100'}`}
            ...
            <div className={`relative flex flex-col items-center gap-6 p-10 border ${borderColor}/20 rounded-lg bg-black/80 backdrop-blur-xl ${shadowColor} transition-all duration-[1500ms]`}>
                <div className={`text-3xl font-bold tracking-[0.3em] uppercase drop-shadow-[0_0_12px_currentColor] flex items-center gap-4 ${themeColor} transition-colors duration-1000`}>
                    {isUnlocking ? <Unlock size={32} /> : <Lock size={32} />}
                    {isUnlocking ? "SYSTEM UNLOCKED" : "SYSTEM LOCKED"}
                </div>

                {/* Camera Feed Frame */}
                <div className={`relative w-64 h-64 border-2 ${borderColor}/30 rounded-lg overflow-hidden bg-black/20 shadow-inner flex items-center justify-center transition-colors duration-500`}>
                    {frameSrc ? (
                        <img
                            src={frameSrc}
                            alt="Auth Camera"
                        ...
                        />
                            ...
                            <div className={`animate-pulse ${isUnlocking ? 'text-green-800' : 'text-orange-900'}`}>
                                <User size={64} />
                            </div>
                        )}
                    )}
                    ...
                    {!isUnlocking && (
                        <div className="absolute top-0 left-0 w-full h-1 bg-orange-400/60 shadow-[0_0_12px_rgba(255,138,0,0.6)] animate-[scan_2s_ease-in-out_infinite]"></div>
                    )}
                ...
                <div className={`text-sm tracking-widest ${isUnlocking ? 'text-green-300' : 'text-orange-300'} animate-pulse transition-colors duration-500`}>
                    {message}
                </div>
            </div>
